...
// getAstraJSONPayloadRest ...
// - HTTP payload for Stargate Astra Rest API
func getAstraJSONPayloadRest(sensorData Util.SensorDataAstra)(string) {
   ...
   for j:=0; j<num; j++ {
      ...
      dataPayload = dataPayload + "\"name\":\"" + strings.ToLower(field.Name) + "\","

      switch value.Kind() {
      case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
         dataPayload = dataPayload + "\"value\":\"" + strconv.FormatInt(value.Int(), 10) + "\""
      case reflect.String:
         dataPayload = dataPayload + "\"value\":\"" + value.String() + "\""  
      case reflect.Float32, reflect.Float64:
         dataPayload = dataPayload + "\"value\":\"" + fmt.Sprintf("%f", value.Float()) + "\""  
      }
...
func writeSensorDataToAstra(
   ...
   svcURL := "https://" + astraDB + "-" + astraRegion + ".apps.astra.datastax.com/api/rest/v1" +
            "/keyspaces/" + astraKs + "/tables/" + astraTbl + "/rows"

   dataPayload := getAstraJSONPayloadRest(sensorData)
